...
### 10. Expose short evidence aliases to the semantic classifier

Long UUID-based evidence IDs are authoritative storage identifiers but unreliable copying targets for a generative model. The classifier prompt uses deterministic aliases (`E001`, `E002`, …) in both evidence and candidates. After structured output parsing, the adapter maps aliases back to authoritative evidence IDs before repair and validation. Raw UUID evidence IDs never need to be generated by the model.
...
- [Risk] Model-generated question suffixes can resemble valid UUID evidence IDs while referring to the wrong question. → Expose only short aliases and resolve membership deterministically after classification.
